...
function stripSkinImages(store){
  // Изображения скинов — только во внешнем каталоге/браузерном localStorage.
  // В постоянном JSON сохраняем лишь идентификаторы и текстовые данные заявок.
  for(const w of (store.withdrawals||[])){
    if(w.item && typeof w.item==='object'){
      delete w.item.img; delete w.item.image; delete w.item.icon_url; delete w.item.image_url;
    }
    delete w.img; delete w.image; delete w.icon_url; delete w.image_url;
  }
  return store;
}
function loadStore(){
  try {
    ...
    return stripSkinImages({users:x.users||{},withdrawals:x.withdrawals||[],deposits:x.deposits||[],promos:x.promos||{},admins:x.admins||{},links:x.links||{}});
  } catch(e) {
    return {users:{},withdrawals:[],deposits:[],promos:{},admins:{},links:{}};
  }
}
function saveStore(){
  try { stripSkinImages(data); fs.writeFileSync(DATA_FILE, JSON.stringify(data,null,2)); } catch(e){ console.error('store save:',e.message); }
}
...
const server = http.createServer(
    async (req, res) => {
        ...
        if(pathname==='/api/withdrawals' && req.method==='POST'){
          ...
          const w={id,steamid:sessionUser.steamid,tgId:stored.tgId||null,index,item:{name:item.name,value:Number(item.value),assetid:item.assetid||null,uid:itemUid},value:Number(item.value),refund:Number(item.value),status:'pending',createdAt:Date.now()};
