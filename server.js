...
// Сессия Steam не должна исчезать после redeploy Render.
// Подпись использует постоянный секрет из Environment Variables.
...
function makeSessionCookie(steamid){
  const payload=String(steamid||'');
  const sig=crypto.createHmac('sha256',SESSION_SECRET).update(payload).digest('hex');
  return `steam_${payload}_${sig}`;
}
...
const server = http.createServer(
    async (req, res) => {
        ...
        if (cookies.session_id && sessions.has(cookies.session_id)) {
            ...
            const steamid=steamFromSessionCookie(cookies.session_id);
            if(steamid){
                const stored=ensureUser(steamid);
                sessionUser={steamid,username:stored.username||'Steam',avatar:stored.avatar||''};
            }
        }
                    ...
                    if (steamId) {
                        ...
                        res.writeHead(
                            302,
                            {
                                ...
                                'Set-Cookie':
                                    `session_id=${sessionId}; Path=/; Max-Age=31536000; HttpOnly; SameSite=Lax; Secure`
                            }
                        );
