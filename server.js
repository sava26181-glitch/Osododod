...
if (!KEY) { console.error('ERROR: Set CS2SH_API_KEY in env'); process.exit(1); }
...
async function redisSave(key,obj){
  try{ await redisCmd(['SET',key,JSON.stringify(obj)]); }catch(e){ console.error('redisSave:',e.message); }
}

// ============================================================
//  КЕЙСЫ — жёсткое окно приёма на кейс
//
//  accept: [minPrice, maxPrice] — скины ВНЕ этого окна вообще не участвуют.
//  bands: распределение внутри окна.
//  Для 10к-кейса accept=[5000, 16000] — 27₽ туда физически не попадут.
// ============================================================
const CASE_CONFIG = {
  micro: {
    ...
    accept: [8, 60],
    bands: [
      { name:'core', min:8,  max:23,  weight:100, label:'часто' },
      { name:'rare', min:23, max:60,  weight:1,   label:'очень редко' }
    ]
  },
  basic: {
    ...
    bands: [
      ...
      { name:'core', min:90,  max:125, weight:100, label:'часто' },
      { name:'rare', min:125, max:250, weight:1.5, label:'редко' }
    ]
  },
  small: {
    ...
    bands: [
      ...
      { name:'core',  min:190, max:280, weight:100, label:'в среднем' },
      { name:'rare',  min:280, max:350, weight:2,   label:'редко' }
    ]
  },
  premium: {
    ...
    bands: [
      ...
      { name:'core',  min:420, max:560, weight:100, label:'часто' },
      { name:'rare',  min:560, max:670, weight:2,   label:'редко' }
    ]
  },
  expensive: {
    ...
    bands: [
      ...
      { name:'core',  min:750,  max:1200, weight:100, label:'дроп' },
      { name:'rare',  min:1200, max:1600, weight:2,   label:'везучий дроп' }
    ]
  },
  elite: {
    ...
    bands: [
      ...
      { name:'rare',  min:2800, max:3200, weight:2,   label:'редко' }
    ]
  },
  legendary: {
    ...
    bands: [
      ...
      { name:'rare',  min:5600, max:6300, weight:2,   label:'редко' }
    ]
  },
  titan: {
    ...
    bands: [
      ...
      { name:'rare',  min:11000, max:16000, weight:2,   label:'редко' }
    ]
  }
};

// Удобные срезы для API и логики
const CASES = {};
const CASE_BANDS = {};
...
// бэнд внутри набора бэндов кейса
...
function computeWeights(items, caseKey, alpha){
  ...
  // 1) Фильтр: только скины в окне accept
  const inWindow = items
    .map(x => ({ ...x, price: itemPrice(x) }))
    .filter(x => x.price >= amin && x.price < amax);

  // 2) Если окно пустое — используем весь пул с крошечными весами (fallback)
  const pool = inWindow.length ? inWindow : items.map(x => ({ ...x, price: itemPrice(x) }));

  // 3) Собираем веса по бэндам
  const arr = pool.map(x => {
    const b = bandOf(x.price, bands);
    // Если скин попал в fallback-пул (не в окне) — вес 0.01
    const w = b
      ? Number(b.weight || 0) * insideBandWeight(x.price, alpha)
      : 0.01 * insideBandWeight(x.price, alpha);
...
// RTP-подгонка (мягкая, допуск 5%). Не может расширить окно.
function fitRtpAbsolute(list, casePrice, targetRtp, iterations = 10){
  ...
  for (let it = 0; it < iterations; it++){
    ...
    l = l.map(x => {
      const exp = x.price >= casePrice ? 1.4 : x.price >= casePrice * 0.6 ? 0.9 : 0.4;
...
function openCase(caseKey, items, pityCount = 0){
  ...
  return {
    skin: { id: picked.id, name: picked.name, img: picked.img, value: picked.price, tier: picked._band },
    price: cfg.price,
    tier: picked._band,
    label: picked._label
  };
...
function saveStoreToDisk(){
  try { fs.writeFileSync(DATA_FILE, JSON.stringify(data,null,2)); } catch(e){ console.error('disk save:',e.message); }
}
...
async function initStore(){
  if(USE_REDIS){
    ...
    if(remote && typeof remote === 'object'){
      ...
      console.log('Store loaded from Redis:', Object.keys(data.users).length,'users,',data.withdrawals.length,'wd,',data.deposits.length,'dep');
      return;
    }
...
function ensureUser(steamid){
  ...
  if(!data.users[id]) data.users[id]={steamid:id,balance:0,stats:{totalDeposited:0,upgradesTotal:0,casesOpened:0},withdrawDisabled:false,tgId:null,createdAt:Date.now(),inventory:[],bestDrop:{name:'--',value:0,img:''},pity:{count:0,byCase:{}}};
...
async function tgWithToken(token,label,method,body={}){
  if(!token){ console.error(`Telegram ${label}: no token`); return null; }
  try{
    const r=await fetch(`https://api.telegram.org/bot${token}/${method}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
    const json=await r.json();
    if(!json?.ok) console.error(`TG ${label} API`,method,json?.description||'err');
    return json;
  }catch(e){ console.error(`TG ${label}`,method,e.message); return null; }
}
...
function promoList(){
  return Object.values(data.promos).filter(x=>x.active!==false && (!x.expiresAt||x.expiresAt>Date.now()) && (!x.maxUses||Number(x.uses||0)<Number(x.maxUses))).sort((a,b)=>Number(b.createdAt||0)-Number(a.createdAt||0));
}
  ...
  if(u.callback_query){
    ...
    if(d.startsWith('wd:')){
      ...
      if(action==='send'){
        ...
        await tg('sendMessage',{chat_id:q.message.chat.id,text:`🎁 <b>${wid}</b> подтверждена. ${w.item?.name}`,parse_mode:'HTML'});
    ...
    if(d.startsWith('dep:')){
      ...
      if(action==='approve'){
        ...
        u.balance+=total; u.stats.totalDeposited=(u.stats.totalDeposited||0)+total;
...
async function processPaymentTelegramUpdate(u){
  if(u.callback_query){
    ...
    if(d==='pay:profile'){
      ...
      if(!user) return tgPay('sendMessage',{chat_id:chatId,text:'Не привязан.'});
      return tgPay('sendMessage',{chat_id:chatId,text:`ID: <code>${ensureZenodropId(user)}</code>\nSteam: <code>${user.steamid}</code>\nБаланс: <b>${Number(user.balance||0).toFixed(2)} ₽</b>`,parse_mode:'HTML'});
    }
  ...
  if(command==='/start'){
    ...
    if(zenoFull){
      ...
      if(user){
        user.tgId=chatId; data.links[chatId]=user.steamid; await saveStoreNow();
  ...
  if(command==='/id'){
    const steam=findSteamByTelegram(chatId), user=steam?ensureUser(steam):null;
    return tgPay('sendMessage',{chat_id:chatId,text:user?`ID: <code>${ensureZenodropId(user)}</code>`:'Не привязан.',parse_mode:'HTML'});
  }
  if(command==='/balance'){
    const steam=findSteamByTelegram(chatId), user=steam?ensureUser(steam):null;
    return tgPay('sendMessage',{chat_id:chatId,text:user?`Баланс: <b>${Number(user.balance||0).toFixed(2)} ₽</b>`:'Не привязан.',parse_mode:'HTML'});
  }
  return sendPaymentMenu(chatId);
}
...
async function sendPaymentMenu(chatId){
  ...
  return tgPay('sendMessage',{chat_id:chatId,text:`<b>Zenodrop — Пополнение</b>\n\n${id?`ID: <code>${id}</code>\nБаланс: <b>${Number(user.balance||0).toFixed(2)} ₽</b>`:'Откройте через сайт.'}`,parse_mode:'HTML',reply_markup:{inline_keyboard:[[{text:'💰 Пополнить',callback_data:'pay:topup'},{text:'👤 Профиль',callback_data:'pay:profile'}]]}});
}

async function paymentTelegramStart(){
  if(!PAY_TG_TOKEN){ console.error('Pay TG disabled'); return; }
  try{
    const me=await tgPay('getMe',{});
    if(!me?.ok){ console.error('Pay TG: bad token'); return; }
    await tgPay('setMyCommands',{commands:[{command:'start',description:'Меню'},{command:'deposit',description:'Пополнить'},{command:'id',description:'ID'},{command:'balance',description:'Баланс'}]});
    if(PAY_TG_WEBHOOK_URL){
      ...
      if(r?.ok) console.log('Pay webhook:',PAY_TG_WEBHOOK_URL); else console.error('Pay setWebhook:',r?.description);
    }
  }catch(e){ console.error('Pay TG init:',e.message); }
}

async function telegramStart(){
  if(!TG_TOKEN){ console.error('Admin TG disabled'); return; }
  try{
    const me=await tg('getMe',{});
    if(!me?.ok){ console.error('TG bad token'); return; }
    await tg('setMyCommands',{commands:[
      {command:'start',description:'Меню'},{command:'panel',description:'Админ-панель'},{command:'deposits',description:'Пополнения'},
      {command:'withdrawals',description:'Выводы'},{command:'stats',description:'Статистика'},{command:'status',description:'Мой баланс'},
      {command:'link',description:'Привязать Steam'},{command:'help',description:'Помощь'}
    ]});
    if(TG_WEBHOOK_URL){
...
function loadCatalogFromDisk(){
  try{
    ...
    if(Array.isArray(x?.items) && x.items.length){
      cs2CatalogCache.data=x.items; cs2CatalogCache.expires=Date.now()+CS2_CACHE_MS;
      console.log('Catalog from disk:',x.items.length); return true;
    }
  }catch(e){}
  return false;
}
...
const server = http.createServer(async (req, res) => {
    ...
    if(pathname==='/api/case/preview' && req.method==='POST'){
        try{
            ...
            let sum=0,tiers={},inWindowCount=0;
            const [amin,amax]=[cfg.acceptMin,cfg.acceptMax];
            for(const s of skins){ const p=Number(s.price??s.value??s.usd??0); if(p>=amin&&p<amax) inWindowCount++; }
            for(let i=0;i<N;i++){
                const r=openCase(caseKey,skins,0); if(!r)continue;
                sum+=r.skin.value; tiers[r.tier]=(tiers[r.tier]||0)+1;
            }
            const ev=sum/N;
            res.writeHead(200,{'Content-Type':'application/json'});
            return res.end(JSON.stringify({ok:true,case:caseKey,samples:N,poolSize:skins.length,inWindow:inWindowCount,accept:[amin,amax],ev:+ev.toFixed(2),evPercent:+((ev/cfg.price)*100).toFixed(2),targetRtp:cfg.rtp,tiers}));
        }catch(e){res.writeHead(400);return res.end(JSON.stringify({error:'invalid_json'}));}
    }
    ...
    if(pathname==='/api/account/sync' && req.method==='POST'){
        ...
        try{
            ...
            if(body.bestDrop&&typeof body.bestDrop==='object'){ u.bestDrop={name:String(body.bestDrop.name||'--'),value:Number(body.bestDrop.value)||0,img:String(body.bestDrop.img||'')}; }
            saveStore();
    ...
    if(pathname==='/api/config' && req.method==='GET'){
        res.writeHead(200,{'Content-Type':'application/json'});
        return res.end(JSON.stringify({telegramBotUrl:TG_BOT_URL||null,paymentTelegramBotUrl:PAY_TG_BOT_URL,cases:CASES,bands:CASE_BANDS,storage:USE_REDIS?'redis':'disk'}));
    }
    ...
    if(pathname==='/telegram/payment-webhook' && req.method==='POST'){
        ...
        if(expected&&provided!==expected){res.writeHead(403,{'Content-Type':'application/json'});res.end(JSON.stringify({ok:false,error:'forbidden'}));return;}
        res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify({ok:true}));
        Promise.resolve(processPaymentTelegramUpdate(update)).catch(e=>console.error('pay wh:',e.message));
        return;
    }
...
(async () => {
  ...
  server.listen(PORT,()=>{
    ...
    if(TG_TOKEN){console.log('Admin TG');telegramStart();}else console.log('Admin TG off');
    if(PAY_TG_TOKEN){console.log('Pay TG');paymentTelegramStart();}else console.log('Pay TG off');
  });
})();
