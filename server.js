...
async function buildCs2Catalog() {
    ...
    // Берём семьи по всему алфавитному диапазону, а не первые N.
    // Так в выборку попадают и дешёвые массовые скины, а не только
    // первые позиции схемы. Для каждой семьи оставляем один wear-вариант.
    const families = new Map();
    for (const x of all) {
        const family = x.name
            .replace(/\s*\((Factory New|Minimal Wear|Field-Tested|Well-Worn|Battle-Scarred)\)$/i, '')
            .toLowerCase();
        if (!families.has(family)) families.set(family, x);
    }

    const familyList = [...families.values()];
    const selected = [];
    const TARGET = 6000;
    if (familyList.length <= TARGET) {
        selected.push(...familyList);
    } else {
        // Равномерная выборка по всему каталогу.
        // В отличие от slice(0, N), она не отсекает дешёвые серии только
        // потому, что они находятся дальше в schema.
        const step = familyList.length / TARGET;
        for (let i = 0; i < TARGET; i++) {
            const idx = Math.min(familyList.length - 1, Math.floor(i * step));
            selected.push(familyList[idx]);
        }
    }

    // Дополнительно гарантируем наличие популярных массовых категорий.
    const cheapKeywords = [
        'glock-18 |', 'usp-s |', 'p250 |', 'p2000 |', 'tec-9 |', 'mp9 |',
        'mac-10 |', 'mp7 |', 'mp5-sd |', 'nova |', 'xm1014 |', 'mag-7 |',
        'famas |', 'galil ar |', 'm4a1-s |', 'm4a4 |', 'ak-47 |', 'awp |',
        'ssg 08 |', 'scar-20 |', 'aug |', 'sg 553 |', 'usp-s |'
    ];
    const selectedNames = new Set(selected.map(x => x.name));
    for (const x of familyList) {
        const low = x.name.toLowerCase();
        if (cheapKeywords.some(k => low.startsWith(k)) && !selectedNames.has(x.name)) {
            selected.push(x);
            selectedNames.add(x.name);
        }
        if (selected.length >= 7000) break;
    }

    console.log('Selected items:', selected.length, 'of', familyList.length, 'families');
