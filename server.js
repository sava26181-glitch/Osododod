...
const PAY_TG_BOT_URL = (process.env.TELEGRAM_PAYMENT_BOT_URL || '').trim();
...
async function processPaymentTelegramUpdate(u){
  ...
  if(command==='/start' || command==='/deposit' || command==='/pay'){
    const payload=raw.split(/\s+/).slice(1).join(' ').trim();
    const deep=payload.toLowerCase().startsWith('deposit');
    return tgPay('sendMessage',{chat_id:chatId,text:'<b>Zenodrop — Пополнение</b>\n\nВаш Telegram ID: <code>'+chatId+'</code>\n\n'+(deep?'Заявка на пополнение через Telegram может быть привязана к этому ID.\n\n':'')+'Откройте сайт Zenodrop и выберите «Пополнение → Telegram». После создания заявки следуйте инструкции на сайте.',parse_mode:'HTML'});
  }
  return tgPay('sendMessage',{chat_id:chatId,text:'Используйте /start для пополнения Zenodrop.',parse_mode:'HTML'});
}
async function paymentTelegramStart(){
  ...
  try{
    ...
    await tgPay('setMyCommands',{commands:[{command:'start',description:'Пополнение Zenodrop'},{command:'deposit',description:'Пополнить баланс'}]});
    if(PAY_TG_WEBHOOK_URL){
      const secret=crypto.createHash('sha256').update(PAY_TG_TOKEN).digest('hex').slice(0,32);
      const r=await tgPay('setWebhook',{url:PAY_TG_WEBHOOK_URL,secret_token:secret,allowed_updates:['message'],drop_pending_updates:false});
