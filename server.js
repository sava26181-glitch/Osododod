...
server.listen(
    ...
    () => {
        ...
        // Старая серверная предзагрузка: каталог собирается сразу после старта Render,
        // чтобы первый посетитель не ждал тяжёлую загрузку schema + batches.
        setTimeout(() => {
                    ...
                    cs2CatalogPromise = buildCs2Catalog()
                        .then(items => {
                            if (items?.length) {
                                ...
                                console.log('CS2.SH catalog preloaded:', items.length);
                            }
                            return items;
                        })
