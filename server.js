...
function isWebAdmin(steamid){return !!data.admins['steam:'+String(steamid)] || TG_ADMIN_IDS.has(String(steamid));}
async function tg(method, body){
  if(!TG_TOKEN)return null;
  try{const r=await fetch(`https://api.telegram.org/bot${TG_TOKEN}/${method}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});return await r.json();}catch(e){console.error('TG',method,e.message);return null;}
}
...
function promoList(){return Object.values(data.promos).filter(x=>x.active!==false && (!x.expiresAt || x.expiresAt>Date.now()));}
function makePromo(code,percent,maxBonus){const c=String(code).toUpperCase().trim();data.promos[c]={code:c,percent:Number(percent)||0,maxBonus:Number(maxBonus)||0,active:true,createdAt:Date.now()};saveStore();return data.promos[c];}
async function processTelegramUpdate(u){
  ...
  const m=u.message; if(!m || !m.chat)return; const chatId=String(m.chat.id), text=String(m.text||'').trim();
  const admin=isTgAdmin(chatId);
  if(text==='/start') return tg('sendMessage',{chat_id:chatId,text:'<b>Zenodrop Bot</b>\n\nВаш Telegram ID: <code>'+chatId+'</code>\nКоманды: /link STEAMID, /status',parse_mode:'HTML'});
  if(text==='/status'){
...
async function telegramLoop(){
  if(!TG_TOKEN || tgLoopRunning)return; tgLoopRunning=true;
  while(tgLoopRunning){
    try{const r=await tg('getUpdates',{offset:tgOffset,timeout:25,allowed_updates:['message','callback_query']});for(const u of (r?.result||[])){tgOffset=Math.max(tgOffset,u.update_id+1);await processTelegramUpdate(u);}}
    catch(e){console.error('Telegram loop:',e.message);await new Promise(r=>setTimeout(r,2000));}
  }
}
    ...
    async (req, res) => {
        ...
        if (
            ...
            if(sessionUser){
              ...
              return res.end(JSON.stringify({...sessionUser,serverAccount:stored,webAdmin:isWebAdmin(sessionUser.steamid)}));
            }
        ...
        if(pathname==='/api/account/sync' && req.method==='POST'){
          ...
          saveStore();res.writeHead(200,{'Content-Type':'application/json'});return res.end(JSON.stringify({ok:true,user:u}));
        }
        if(pathname==='/api/promos' && req.method==='GET'){
          const list=promoList().slice(0,5).map(x=>({code:x.code,percent:x.percent,maxBonus:x.maxBonus}));
        ...
        if(pathname==='/api/deposit' && req.method==='POST'){
          ...
          let bonus=0; const pc=data.promos[promo]; if(pc?.active!==false){bonus=Math.min(amount*(Number(pc.percent)||0)/100,Number(pc.maxBonus)||Infinity);pc.uses=(pc.uses||0)+1;}
          ...
          res.writeHead(200,{'Content-Type':'application/json'});return res.end(JSON.stringify({ok:true,id,bonus}));
        }
        if(pathname==='/api/withdrawals' && req.method==='POST'){
          ...
          // Items are held in the browser currently; the server stores the withdrawal request and Telegram admin review.
          ...
          const w={id,steamid:sessionUser.steamid,tgId:stored.tgId||null,index,item:{name:item.name,value:Number(item.value),img:item.img||'',assetid:item.assetid||null},value:Number(item.value),refund:Number(item.value),status:'pending',createdAt:Date.now()};
        ...
        if(pathname==='/api/admin/deposit-confirm' && req.method==='POST'){
          ...
          if(d.status!=='pending'){return res.end(JSON.stringify({ok:true,status:d.status}));} const u=ensureUser(d.steamid);u.balance+=d.amount+d.bonus;u.stats.totalDeposited=(u.stats.totalDeposited||0)+d.amount+d.bonus;d.status='paid';d.updatedAt=Date.now();saveStore();
...
server.listen(
    ...
    () => {
        ...
        if(TG_TOKEN){ console.log('Telegram bot enabled'); telegramLoop(); }
    }
);
