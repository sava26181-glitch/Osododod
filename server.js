...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/api/auth/google' && req.method === 'POST') {
    try {
      ...
      const serverAccount = {...stored, pendingWithdrawals: pendingWithdrawalsForUser(accountId)};
      ...
      res.writeHead(200,{
        ...
        'Set-Cookie':`session_id=${sid}; Path=/; HttpOnly;${securePart} SameSite=Lax; Max-Age=31536000`
      });
      return res.end(JSON.stringify({ok:true,user:{...userData,serverAccount,webAdmin:isWebAdmin(accountId),luck:getPlayerLuck(accountId)}}));
    } catch(e) {
