...
function makeZenodropId(steamid){
  const sid=String(steamid||'');
  if(/^\d{17}$/.test(sid)){
    const hex=crypto.createHash('sha256').update('zenodrop:'+sid).digest('hex').slice(0,12);
    return 'ZN-'+String((parseInt(hex,16)%90000000)+10000000);
  }
  let id='';
...
function ensureZenodropId(user){
  // Существующий ID не меняем. Для нового Steam-аккаунта ID стабильно зависит от SteamID.
  if(!user.zenoId || !/^ZN-\d{8}$/.test(String(user.zenoId))) user.zenoId=makeZenodropId(user.steamid);
  return user.zenoId;
}
