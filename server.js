...
function promoList(){return Object.values(data.promos).filter(x=>x.active!==false && (!x.expiresAt || x.expiresAt>Date.now())).sort((a,b)=>Number(b.createdAt||0)-Number(a.createdAt||0));}
function makePromo(code,percent,maxBonus,extra={}){const c=String(code).toUpperCase().trim();data.promos[c]={code:c,percent:Number(percent)||0,maxBonus:Number(maxBonus)||0,active:true,createdAt:Date.now(),...extra};saveStore();return data.promos[c];}
function randomPromoCode(){return 'ZENO-'+crypto.randomBytes(4).toString('hex').toUpperCase();}
function createAutoPromo(){
  const now=Date.now();
  for(const p of Object.values(data.promos)){ if(p.auto && p.expiresAt && p.expiresAt<=now) p.active=false; }
  const percent=10+crypto.randomInt(0,14);
  const code=randomPromoCode();
  const p=makePromo(code,percent,0,{auto:true,expiresAt:now+15*60*1000});
  console.log('New auto promo:',p.code,p.percent+'%');
  return p;
}
if(!promoList().some(x=>x.auto)){createAutoPromo();}
setInterval(createAutoPromo,15*60*1000);
