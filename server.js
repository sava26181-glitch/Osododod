...
function makeSessionCookie(accountId){
  const id=String(accountId||'');
  const sig=crypto.createHmac('sha256',SESSION_SECRET).update(id).digest('hex');
  return `zeno_${Buffer.from(id).toString('base64url')}_${sig}`;
}
function accountFromSessionCookie(value){
  const v=String(value||'');
  const m=v.match(/^zeno_([A-Za-z0-9_-]+)_([a-f0-9]{64})$/);
  if(!m)return null;
  let id='';
  try{id=Buffer.from(m[1],'base64url').toString('utf8');}catch(e){return null;}
  if(!id)return null;
...
function makeGoogleStateCookie(state, verifier){
  const payload = `${state}.${verifier}`;
  const sig = crypto.createHmac('sha256', SESSION_SECRET).update(payload).digest('hex');
  return `gstate_${Buffer.from(payload).toString('base64url')}_${sig}`;
}
function readGoogleStateCookie(value){
  const m=String(value||'').match(/^gstate_([A-Za-z0-9_-]+)_([a-f0-9]{64})$/);
  if(!m)return null;
  let payload=''; try{payload=Buffer.from(m[1],'base64url').toString('utf8')}catch(e){return null;}
  const expected=crypto.createHmac('sha256',SESSION_SECRET).update(payload).digest('hex');
  try{if(!crypto.timingSafeEqual(Buffer.from(m[2]),Buffer.from(expected)))return null;}catch(e){return null;}
  const parts=payload.split('.'); if(parts.length!==2)return null;
...
async function processTelegramUpdate(u){
  ...
  let a=text.match(/^\/give\s+(ZN-?\d{6,10})\s+([\d.]+)/i);
  if(a){
    await refreshFromRedis();
    const u2=findUserByZenodropId(a[1]);
    if(!u2) return tg('sendMessage',{chat_id:chatId,text:'❌ Пользователь с таким Zenodrop ID не найден.'});
    const amount=Number(a[2]);
    if(!Number.isFinite(amount) || amount<=0) return tg('sendMessage',{chat_id:chatId,text:'❌ Некорректная сумма.'});
    ensureZenodropId(u2);
    u2.balance+=amount;
    await saveStoreNow();
    return tg('sendMessage',{chat_id:chatId,text:`✅ <b>${u2.zenoId}</b>\nБаланс: +${amount.toFixed(2)} ₽\nИтого: ${Number(u2.balance).toFixed(2)} ₽`,parse_mode:'HTML'});
  }
  a=text.match(/^\/withdrawlock\s+(ZN-?\d{6,10})\s+(on|off)/i);
  if(a){
    await refreshFromRedis();
    const u2=findUserByZenodropId(a[1]);
    if(!u2) return tg('sendMessage',{chat_id:chatId,text:'❌ Пользователь с таким Zenodrop ID не найден.'});
    ...
    await saveStoreNow();
    return tg('sendMessage',{chat_id:chatId,text:`✅ <b>${u2.zenoId}</b>\nВывод: ${u2.withdrawDisabled?'OFF':'ON'}`,parse_mode:'HTML'});
  }
...
async function buildCs2Catalog() {
    ...
    const rest=candidates.filter(x=>!selectedSet.has(x.name));
    ...
    for(const x of selected){
        const p=priceItems[x.name]; if(!p)continue;
        let usd=0;
        for(const src of sources){ const ask=Number(p?.[src]?.ask); if(Number.isFinite(ask)&&ask>0){usd=ask;break;} }
        if(usd<=0)continue;
...
const server = http.createServer(async (req, res) => {
  ...
  if (sid) {
    if (sessions.has(sid)) {
      ...
      if (accountId) {
        ...
        if (stored) {
          sessionUser = { steamid: accountId, username: stored.username || '', avatar: stored.avatar || '', email: stored.email || '' };
  ...
  if (pathname === '/auth/google' && req.method === 'GET') {
    if(!googleConfigured()){
      ...
      return res.end('Google OAuth не настроен: задайте GOOGLE_CLIENT_ID.');
    }
    const state=base64url(crypto.randomBytes(32));
    const verifier=base64url(crypto.randomBytes(48));
    googleOAuthStates.set(state,{verifier,createdAt:Date.now()});
    for(const [k,v] of googleOAuthStates){ if(Date.now()-v.createdAt>10*60*1000) googleOAuthStates.delete(k); }
  ...
  if (pathname === '/auth/google/callback' && req.method === 'GET') {
    try{
      ...
      const userData={steamid:accountId,username:info.name||info.email?.split('@')[0]||'Google User',avatar:info.picture||'',email:info.email||''};
      const newsid=makeSessionCookie(accountId);
      sessions.set(newsid,userData);
      const stored=ensureUser(accountId);
      ensureZenodropId(stored);
      stored.username=userData.username; stored.avatar=userData.avatar; stored.email=userData.email;
      stored.authProvider='google'; stored.googleSub=String(info.sub);
      try{ await saveStoreNow(); }catch(saveErr){ console.error('Google account save warning:',saveErr); }
      res.writeHead(302,{
        'Location':googleBaseUrl(req)+'/#profile',
        'Cache-Control':'no-store',
        'Set-Cookie':[
          `session_id=${newsid}; Path=/; HttpOnly;${SECURE_FLAG} SameSite=Lax; Max-Age=31536000`,
    ...
    try {
      ...
      if(vt.includes('is_valid:true')){
        ...
        if(steamId){
          ...
          const newsid = makeSessionCookie(steamId);
          sessions.set(newsid,userData);
          const stored = ensureUser(steamId); ensureZenodropId(stored); stored.username=userData.username; stored.avatar=userData.avatar;
          await saveStoreNow();
          res.writeHead(302,{Location:'/','Set-Cookie':`session_id=${newsid}; Path=/; HttpOnly;${SECURE_FLAG} SameSite=Lax; Max-Age=31536000`});
          return res.end();
        }
      }
  ...
  if(pathname==='/api/admin/user' && req.method==='POST'){
    ...
    const body = await readJson(req);
    await refreshFromRedis();
    const zenoId = String(body.zenoId||'').trim();
    const u = zenoId ? findUserByZenodropId(zenoId) : null;
    if(!u){ res.writeHead(400); return res.end(JSON.stringify({error:'zenodrop_id_not_found'})); }
    ensureZenodropId(u);
