...
function ensureDefaultPromos(){
  const defaults=[['WELCOME10',10],['ZENODROP15',15],['DROP20',20]];
  for(const [code,pct] of defaults){
    if(!data.promos[code]) makePromo(code,pct,0,{auto:false});
  }
}
ensureDefaultPromos();
...
async function telegramLoop(){
  ...
  try{
    ...
    if(!me?.ok){console.error('Telegram bot initialization failed');tgLoopRunning=false;return;}
    tgUsername=me.result.username||'';
    console.log('Telegram bot:',tgUsername?'@'+tgUsername:me.result.first_name||'unknown');
    await tg('setMyCommands',{commands:[{command:'start',description:'Открыть бота'},{command:'status',description:'Показать баланс'},{command:'link',description:'Привязать Steam ID'},{command:'help',description:'Список команд'}]});
    if(TG_WEBHOOK_URL){
      const hook=await tg('getWebhookInfo',{});
      if(hook?.ok && hook.result?.url!==TG_WEBHOOK_URL){
        const set=await tg('setWebhook',{url:TG_WEBHOOK_URL,allowed_updates:['message','callback_query'],drop_pending_updates:false});
        if(set?.ok){ console.log('Telegram webhook enabled:',TG_WEBHOOK_URL); tgLoopRunning=false; return; }
        console.error('Telegram webhook setup failed, falling back to long polling');
      }else if(hook?.ok){
        console.log('Telegram webhook already active'); tgLoopRunning=false; return;
      }
    }
    const hook=await tg('getWebhookInfo',{});
    if(hook?.ok && hook.result?.url){
      console.log('Removing Telegram webhook for long polling');
  ...
  while(tgLoopRunning){
    try{
      const r=await tg('getUpdates',{offset:tgOffset,timeout:25,allowed_updates:['message','callback_query']});
      if(r?.ok){
    ...
    }catch(e){console.error('Telegram loop:',e.message);await new Promise(r=>setTimeout(r,3000));}
  }
}
...
const server = http.createServer(
    async (req, res) => {
        ...
        if(pathname==='/telegram/webhook' && req.method==='POST'){
          ...
          Promise.resolve(processTelegramUpdate(update)).catch(e=>console.error('Telegram webhook:',e.message));
          return;
        }
