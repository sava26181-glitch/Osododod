...
async function telegramLoop(){
  ...
  // Используем только long polling. Webhook на Render мог оставаться
  // от предыдущей версии и блокировать getUpdates. Telegram не позволяет
  // одновременно получать обновления через webhook и getUpdates.
  try{
    ...
    // Всегда удаляем старый webhook перед polling.
    ...
    // Подхватываем только новые сообщения, чтобы старый мусор не спамил бота.
    const pending=await tg('getUpdates',{offset:-1,timeout:0,allowed_updates:['message','callback_query']});
    if(pending?.ok && pending.result?.length){
      tgOffset=pending.result[pending.result.length-1].update_id+1;
    }
    ...
    try{
      ...
      for(const u of (r.result||[])){
        ...
        try{
          ...
          console.error('Telegram update:',e.message);
        }
      }
    }catch(e){
...
const server = http.createServer(
    async (req, res) => {
        ...
        if(pathname==='/telegram/webhook' && req.method==='POST'){
          ...
          // Обрабатываем update сразу, чтобы ответ /start гарантированно ушёл.
