...
function googleBaseUrl(req){
  const configured=(PUBLIC_URL || '').replace(/\/$/,'');
  if(configured) return configured;
  if(req.headers.host === 'zenodrop.fun' || req.headers.host === 'www.zenodrop.fun') return 'https://zenodrop.fun';
  const proto=req.headers['x-forwarded-proto'] || 'https';
  const host=req.headers['x-forwarded-host'] || req.headers.host || 'zenodrop.fun';
  return `${proto}://${host}`;
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/auth/google/callback' && req.method === 'GET') {
    try{
      ...
      if(oauthErr) return res.writeHead(302,{Location:'/?google_error='+encodeURIComponent(oauthErr)}),res.end();
      if(!code || !stateData || Date.now()-stateData.createdAt>10*60*1000){
        return res.writeHead(302,{Location:'/?google_error=invalid_callback'}),res.end();
      }
      const redirectUri=googleBaseUrl(req)+'/auth/google/callback';
      ...
      const sid=makeSessionCookie(accountId);
      ...
      res.writeHead(302,{
        ...
        'Set-Cookie':`session_id=${sid}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=31536000`
      });
      return res.end();
    }catch(e){
      console.error('Google OAuth callback:',e);
      res.writeHead(302,{Location:'/?google_error=auth_failed', 'Cache-Control':'no-store'});
      return res.end();
    }
  }
  ...
  if (pathname === '/api/auth/google' && req.method === 'POST') {
    try {
      if(!googleConfigured()) {
        ...
        return res.end(JSON.stringify({error:'google_not_configured'}));
      }
      ...
      if(!verifyResp.ok || !info.sub || info.aud !== GOOGLE_CLIENT_ID || (info.email_verified && info.email_verified !== 'true')) {
        console.error('Google GIS token rejected:', info);
        res.writeHead(401,{'Content-Type':'application/json'});
        return res.end(JSON.stringify({error:'invalid_google_token'}));
      }
      if(info.iss && info.iss!=='https://accounts.google.com' && info.iss!=='accounts.google.com') {
        res.writeHead(401,{'Content-Type':'application/json'});
        return res.end(JSON.stringify({error:'invalid_issuer'}));
      }

      const accountId=`google_${String(info.sub)}`;
      const userData={
        steamid:accountId,
        username:info.name||info.email?.split('@')[0]||'Google User',
        avatar:info.picture||'',
        email:info.email||''
      };
      const sid=makeSessionCookie(accountId);
      ...
      stored.email=userData.email;
      stored.authProvider='google';
      stored.googleSub=String(info.sub);
      await saveStoreNow();
      ...
      return res.end(JSON.stringify({
        ok:true,
        user:{...userData, serverAccount:responseAccount, webAdmin:isWebAdmin(accountId), luck:getPlayerLuck(accountId)}
      }));
    } catch(e) {
      console.error('Google GIS auth:',e);
      res.writeHead(500,{'Content-Type':'application/json'});
      return res.end(JSON.stringify({error:'google_auth_failed'}));
    }
  }
  ...
  if (pathname === '/') { res.writeHead(200,{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store'}); return res.end(html); }
