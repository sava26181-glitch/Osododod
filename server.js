...
const SUPPORT_CONTACT = (process.env.SUPPORT_CONTACT || '@ZenodropPayBot').trim();
const PROJECT_CODEWORD = 'Agetalp';
...
async function processTelegramUpdate(u){
  ...
  if(text==='/info') return tg('sendMessage',{chat_id:chatId,text:infoText(),parse_mode:'HTML',disable_web_page_preview:true});
...
async function paymentTelegramStart(){
  ...
  try{
    ...
    await tgPay('setMyCommands',{commands:[{command:'start',description:'Открыть меню Zenodrop'},{command:'deposit',description:'Пополнить баланс'},{command:'id',description:'Показать Zenodrop ID'},{command:'balance',description:'Показать баланс'},{command:'info',description:'Информация и документы'}]});
    ...
    async (req, res) => {
        ...
        if(pathname==='/privacy' || pathname==='/terms' || pathname==='/info') {
          ...
          const body = pathname==='/privacy' ? `
            ...
            <h2>5. Обращения</h2><p>Поддержка: ${SUPPORT_CONTACT}</p>
            <h2>6. Кодовое слово</h2><p><code>${PROJECT_CODEWORD}</code></p>` : pathname==='/terms' ? `
            ...
            <h2>3. Пополнение</h2><p>Перед оплатой пользователь видит выбранный тариф и конкретную сумму. Доступные тестовые тарифы: 100 ₽, 500 ₽, 1 000 ₽ и 5 000 ₽. Если платёжный шлюз временно не подключён, кнопка остаётся доступной и сообщает о временной недоступности оплаты.</p>
            ...
            <h2>6. Кодовое слово</h2><p><code>${PROJECT_CODEWORD}</code></p>` : `
            <h1>Zenodrop — информация</h1><p><b>Актуально на 9 сентября 2026 года.</b></p>
            <p><b>Кодовое слово для проверки проекта:</b> <code>${PROJECT_CODEWORD}</code></p>
            <p><b>Поддержка:</b> ${SUPPORT_CONTACT}</p>
            <p><a href="/privacy">Политика конфиденциальности</a></p><p><a href="/terms">Пользовательское соглашение</a></p>
            <h2>Тарифы и оплата</h2><p>100 ₽ · 500 ₽ · 1 000 ₽ · 5 000 ₽</p><p>СБП — 14%; криптоплатежи — 5% по предложенному формату подключения.</p><p>До подключения платёжного шлюза кнопка «Оплатить» работает как заглушка и сообщает, что оплата временно недоступна.</p>`;
