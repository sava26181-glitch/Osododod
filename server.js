...
const dbPool = DATABASE_URL && /^(postgres|postgresql):\/\//i.test(DATABASE_URL) ? new Pool({
...
async function loadCatalogFromDatabase(){
  if(!dbPool || dbDisabled) return null;
  try{
  ...
  }catch(e){ console.error('PostgreSQL catalog read:', e.message); if(/ENOTFOUND|ECONNREFUSED|ENETUNREACH|ETIMEDOUT/i.test(String(e?.message||e))) dbDisabled=true; }
  return null;
}

async function saveCatalogToDatabase(items){
  if(!dbPool || dbDisabled || !Array.isArray(items) || !items.length) return;
  try{
  ...
  }catch(e){ console.error('PostgreSQL catalog save:', e.message); if(/ENOTFOUND|ECONNREFUSED|ENETUNREACH|ETIMEDOUT/i.test(String(e?.message||e))) dbDisabled=true; }
}
...
async function buildCs2Catalog() {
    ...
    let cursor = 0;
    // CS2.SH rate-limits aggressive parallel requests. Keep concurrency low
    // and retry temporary 429/5xx responses with exponential backoff.
        ...
        while (true) {
            ...
            for (let attempt = 0; attempt < 4 && !done; attempt++) {
                try {
                    ...
                    const retryable = status === 429 || status >= 500 || /rate.?limit/i.test(String(e?.message || ''));
                    if (!retryable || attempt === 3) {
                        console.warn('CS2 batch failed:', e.message || e);
                        break;
                    }
                    const wait = Math.min(15000, 1500 * (2 ** attempt));
                    console.warn(`CS2 batch ${i + 1}/${batches.length} rate limited; retrying in ${wait}ms`);
                    await new Promise(resolve => setTimeout(resolve, wait));
                }
            }
            // Small gap between batches even after success.
...
dbReady = initDatabase().catch(e => {
  console.error('Database startup error:', e?.message || e);
});
